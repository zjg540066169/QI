...
class Attention_GRU(torch.nn.Module):
    ...
    def forward(self, word_input):
        ...
        decoder_output,decoder_hidden = self.decode(torch.zeros(self.output_size,1),encoder_hidden,encoder_output)
...
class Train_Net(object):
    def __init__(self,path,save_path,pca_path):
        ...
        data_proces.process()
        self.X,self.Y = data_proces.get_X(),data_proces.get_Y().iloc[:,-1]
