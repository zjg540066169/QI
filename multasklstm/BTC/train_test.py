...
if __name__ == '__main__':
    ...
    run_time = []
    task = 4
    for input_length,last_length,predict_length in zip([24,48,96],[4,16,32],[1,4,8]):
        train = []
        val = [] 
        running_time = []
        for i in ['FC','LSTM','LSTM-attention','Self-attention']:
            if i == 'FC':
        ...
        plt.plot(np.arange(epochnum),train[2],label='LSTM-attention')
        ...
        plt.plot(np.arange(epochnum),val[2],label='LSTM-attention')
