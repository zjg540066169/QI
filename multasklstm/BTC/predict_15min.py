...
if __name__=='__main__':
    ...
    while True:
        ...
        if (timeNow.minute + send_time) % 15 == 0:
            ...
            string += '未来1小时破最高'+str(data.loc[data.index[-2],"022_HH"])+"的概率是"+str('%.2f%%' %(100 * predict_prob[2][0][0]))+'\n'
            string += '未来1小时破次高'+str(data.loc[data.index[-2],"022_fb1"])+"的概率是"+str('%.2f%%' %(100 * predict_prob[2][0][1]))+'\n'
            string += '未来1小时破最低'+str(data.loc[data.index[-2],"022_LL"])+"的概率是"+str('%.2f%%' %(100 * predict_prob[3][0][0]))+'\n'
            string += '未来1小时破次低'+str(data.loc[data.index[-2],"022_fb4"])+"的概率是"+str('%.2f%%' %(100 * predict_prob[3][0][1]))+'\n'
