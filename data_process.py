...
class data_process(object):
    def __init__(self,path,pcapath,MulEncoding = 13,ERROR_RATE = 0, ERROR_RATE_Vol = 0.2, input_length = 96,predict_length = 4,last_length = 16, long_input = False):
        ...
        self.data = self.data.iloc[int(len(self.data)/10)*1:int(len(self.data)/10)*2,:]
...
# =============================================================================
        ...
        self.train_y_data = train.loc[:,'tag'].copy()
        self.train_y_data[self.train_y_data == -1] = 2
        deleted_columns = ['tag','high','low','open','close','hl2','close_ema_5','close_ema_13',"close_ema_30",'close_ema_75']#['022_HH','022_LL','022_med','022_fb1','022_fb2','022_fb3','022_fb4','high','low','open','close','hl2','volume','volume_sma_5','close_ema_5','close_ema_13',"close_ema_30",'close_ema_75']
            ...
            if train.index[i+input_length] in index: 
                for j in range(-2,3):
                    try:
                        ...
                        train_y.append(self.train_y_data.iloc[i+input_length+j])
...
if __name__ == '__main__':
    data_proces = data_process(r'../data/bitfinex_2017-01-01_to_2019-01-14_eth_usdt_15m_singal_regular_20190526.json',r'BTC/15PCA.m')
